# 健康診断結果ツール ユーザーストーリー・バックログ

> **作成日:** 2026-04-03  
> **担当:** po_agent

---

## ユーザーストーリー

### Epic 1: 診断結果の記録

| ID | ストーリー | 優先度 |
|----|-----------|--------|
| US-01 | ユーザーとして、健康診断結果の用紙をカメラで撮影することで、手入力なしにデータを記録したい | 最高 |
| US-02 | ユーザーとして、用紙が大きい場合に複数枚撮影してまとめてOCR処理したい | 高 |
| US-03 | ユーザーとして、OCRの読み取り結果を確認・修正してから保存したい | 最高 |
| US-04 | ユーザーとして、OCRが失敗した場合にその理由を知り、手動で値を入力したい | 高 |
| US-05 | ユーザーとして、医療機関が変わっても柔軟に検査項目を登録・管理したい | 高 |

### Epic 2: 結果の閲覧・比較

| ID | ストーリー | 優先度 |
|----|-----------|--------|
| US-06 | ユーザーとして、過去の診断記録を一覧で確認したい | 最高 |
| US-07 | ユーザーとして、特定の検査項目の経年変化をグラフで確認したい | 高 |
| US-08 | ユーザーとして、基準値と自分の数値を並べて確認したい | 高 |

### Epic 3: アラート・通知

| ID | ストーリー | 優先度 |
|----|-----------|--------|
| US-09 | ユーザーとして、基準値を外れた検査項目があるときに通知で知りたい | 中 |
| US-10 | ユーザーとして、通知一覧で基準値外の項目をまとめて確認したい | 中 |

### Epic 5: Web版 – 手動入力・閲覧

| ID | ストーリー | 優先度 |
|----|-----------|--------|
| US-W01 | ユーザーとして、PCブラウザからGoogleアカウントでログインして健診記録を管理したい | 最高 |
| US-W02 | ユーザーとして、Webから検査日・施設名・検査項目を手動で入力して記録を追加したい | 最高 |
| US-W03 | ユーザーとして、Web上でAndroidで入力した記録も含めて一覧・詳細を確認したい | 最高 |
| US-W04 | ユーザーとして、Web上でも検査項目の経年グラフを確認したい | 高 |
| US-W05 | ユーザーとして、基準値外の項目がある記録を一覧で把握したい | 中 |

### Epic 6: クロスプラットフォーム同期

| ID | ストーリー | 優先度 |
|----|-----------|--------|
| US-S01 | ユーザーとして、AndroidとWebで同じアカウントを使い、どちらで入力しても同じデータを参照したい | 最高 |
| US-S02 | ユーザーとして、オフラインでAndroidに入力したデータが、オンラインに戻ったときに自動でWeb側にも反映されてほしい | 中 |

### Epic 4: 認証・データ管理

| ID | ストーリー | 優先度 |
|----|-----------|--------|
| US-11 | ユーザーとして、Googleアカウントでログインして自分のデータを安全に管理したい | 中 |
| US-12 | ユーザーとして、複数デバイスでデータを同期したい（将来対応） | 低 |

---

## プロダクトバックログ

### Sprint 1（4/3〜4/8）: 設計・スコープ確定 + プロトタイプ基盤

| チケット | タスク | 担当 | ステータス |
|---------|--------|------|-----------|
| T-001 | 技術選定・アーキテクチャ設計ドキュメント作成 | po_agent | ✅ 完了 |
| T-002 | ユーザーストーリー・バックログ作成 | po_agent | ✅ 完了 |
| T-003 | Androidプロジェクト初期構成（依存関係定義） | engineer | ✅ 完了 |
| T-004 | CameraX基本実装（撮影フロー） | engineer | ✅ 完了 |
| T-005 | ML Kit Text Recognition 動作確認 | engineer | ✅ 完了 |
| T-006 | GitHubリポジトリ作成・初回push | zakis | ✅ 完了 |

### Sprint 2（4/9〜）: MVP コア機能

| チケット | タスク | 担当 | ステータス |
|---------|--------|------|-----------|
| T-101 | OCR結果確認・手動補正画面 | engineer | ✅ 完了 |
| T-102 | Room DB 設計・実装（ExaminationRecord / Item） | engineer | ✅ 完了 |
| T-103 | 記録一覧・詳細画面 | engineer | ✅ 完了 |
| T-104 | 項目マスター管理（基準値設定） | engineer | ✅ 完了 |
| T-105 | OCRエラーハンドリング実装 | engineer | ✅ 完了 |

### Sprint 3（4/4）: グラフ・通知

| チケット | タスク | 担当 | ステータス |
|---------|--------|------|-----------|
| T-201 | MPAndroidChart 経年グラフ実装 | engineer | ✅ 完了 |
| T-202 | 基準値外アラート通知 | engineer | ✅ 完了 |
| T-203 | 通知一覧画面 | engineer | ✅ 完了 |

### Sprint 4（4/4）: 認証・仕上げ

| チケット | タスク | 担当 | ステータス |
|---------|--------|------|-----------|
| T-301 | Firebase Auth Google SSO 実装 | engineer | ✅ 完了 |
| T-302 | UI/UX ブラッシュアップ | engineer | ✅ 完了 |
| T-303 | 薬事法コンプライアンス確認・ドキュメント化 | po_agent | ✅ 完了 |
| T-304 | Google Play ストア申請準備 | po_agent | ✅ 完了 |

### Sprint 5: Android Firestore同期

| チケット | タスク | 担当 | ステータス |
|---------|--------|------|-----------|
| T-401 | Firestore SDK追加・セキュリティルール設定 | engineer | ✅ 完了 |
| T-402 | Repository層にFirestore書き込み追加（Room→Firestore同期） | engineer | ✅ 完了 |
| T-403 | Firestore→Room の初回ログイン時データ取得（他端末データ復元） | engineer | ✅ 完了 |

### Sprint 6: Web版MVP（手動入力・閲覧）

| チケット | タスク | 担当 | ステータス |
|---------|--------|------|-----------|
| T-501 | Webプロジェクト初期構成（React + TypeScript + Vite + Firebase） | engineer | ✅ 完了 |
| T-502 | Firebase Auth Google SSOログイン画面 | engineer | ✅ 完了 |
| T-503 | 記録一覧・詳細閲覧画面（Firestore連携） | engineer | ✅ 完了 |
| T-504 | 手動入力フォーム（検査日・施設名・項目追加） | engineer | ✅ 完了 |
| T-505 | 項目マスター管理（基準値設定） | engineer | ✅ 完了 |

### Sprint 8（2026-07-18〜）: 画面遷移・画面仕様刷新（specs/001-screen-flow-renewal/）

| チケット | タスク | 担当 | ステータス |
|---------|--------|------|-----------|
| T-701 | ドキュメント整備（specs/DESIGN.md/構成図/README/CLAUDE.md） | engineer | ✅ 完了 |
| T-702 | テスト基盤+リグレッションテスト（OcrParser/DAO/nav_graph/CI） | engineer | ✅ 完了 |
| T-703 | ItemMaster拡張（category/isFavorite）+ Room v2 Migration + マスタ25項目 | engineer | ✅ 完了 |
| T-704 | 画面遷移刷新（S-02ホームハブ・S-05登録方法選択・nav_graph） | engineer | ✅ 完了 |
| T-705 | S-03項目一覧（カテゴリ色・お気に入り）/ S-04期間タブ・最新値 | engineer | ✅ 完了 |
| T-706 | S-06b手入力（BMI自動計算）/ S-06a ⊕項目追加 | engineer | ✅ 完了 |
| T-707 | S-07お問い合わせ（mailto） | engineer | ✅ 完了 |
| T-708 | 実機/エミュレータ通し確認（S-01〜S-07） | zakis | 未着手 |
| T-709 | contact_email 実アドレス設定（現在プレースホルダ） | po_agent | 未着手 |

### Sprint 7: Web版グラフ・仕上げ

| チケット | タスク | 担当 | ステータス |
|---------|--------|------|-----------|
| T-601 | 経年グラフ（Chart.js） | engineer | 🔍 レビュー待ち（Issue #28, PR #42） |
| T-602 | 基準値外一覧画面 | engineer | ✅ 完了 |
| T-603 | Firebase Hostingデプロイ設定 | engineer | 未着手 |
| T-604 | Firestoreセキュリティルール最終確認 | engineer | 未着手 |

### Sprint 9: データ削除・整合性対応（Issue駆動）

| チケット | タスク | 担当 | ステータス |
|---------|--------|------|-----------|
| T-901 | Android 記録詳細画面に記録削除導線を追加（確認ダイアログ必須・Room/Firestore双方から削除）(Issue #47) | engineer | 🔍 レビュー待ち（PR作成済み） |

---

## 定義: 完了の定義（DoD）

- コードレビュー完了
- 単体テスト記述済み（OCRパーサーは必須）
- 医療アドバイス・薬事法違反コンテンツが含まれていないことを確認
- OCRエラー時のユーザーフィードバックが実装されていること

---

## リスク・前提条件

| リスク | 対策 |
|--------|------|
| 日本語OCR精度（医療用語・数値の誤読） | 確認UIで必ず人間がレビューする設計で対処 |
| 医療機関ごとの用紙フォーマット多様性 | 動的Key-Value型で吸収。MVP後にフォーマットテンプレート機能を検討 |
| 薬事法・医療機器規制 | データ表示のみ（診断・アドバイス機能なし）を設計レベルで担保 |
| Firebase依存 | Auth のみ使用。DB はローカルRoomで独立性を確保 |
